...
[[doc:graph][Graphs]] are models for relations.
...
Since $\src, \tgt$ are doc:univalent ---partial functions---, but not
   ...
   - Note that $Ξ = \src ⨾ \src ˘ ∩ \tgt ⨾ \tgt˘$ is /almost/ an doc:Equivalence relation, which
     ...
     It is /not/ an equivalence since it may not be reflexive: Partial edges are
     not parallel with each other nor themselves! ---Since parallelism requires
     the /existence/ of /both/ start and end points.
