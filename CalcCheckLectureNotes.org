...
* COMMENT Organising calculations
  :PROPERTIES:
  :CUSTOM_ID: COMMENT-Organising-calculations
  :END:

  Just as in doing arithmetic, we have the freedom to choose the most
  effective way to organise our calculations.
...
A Boolean expression not equal to true is equal to false ---indeed, it is a
simple proof to show ~(p ≢ true) ≡ (p ≡ false)~.
