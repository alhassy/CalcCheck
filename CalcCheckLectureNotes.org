...
#+options: tags:nil d:nil toc:nil
...
1. Quantification let's us “sum a bunch of /numeric/ values” or “or a bunch of
  /Boolean/ values”.
...
** Rules about Quantification
   :PROPERTIES:
   :CUSTOM_ID: Rules-about-Quantification
   :END:

 # See LADM p148 for Leibniz rules for quantifiers.

Suppose ⊕ is associative, symmetric, having unit $Id_⊕_$.

*** Introduction & Elimination of quantifiers
    :PROPERTIES:
...
*** Generalised Symmetry
    :PROPERTIES:
    :CUSTOM_ID: Generalised-Symmetry
    :END:

| (8.15) Axiom, Distributivity (“Rearrangement”)          |
| $(⊕ x ❙ R • P ⊕ Q) \;=\; (⊕ x ❙ R • P) ⊕ (⊕ x ❙ R • Q)$ |

This holds since ⊕ is symmetric and associative, so that the order in which the
operands are accumulated has no bearing on the result.
- The dummies and ranges are all the same in each quantification in (8.15).

*Proviso:* For (8.15), we need quantification to be defined.

Otherwise, we could write $(Σ i • 0) = (Σ i • i - i) = (Σ i • i) + (Σ i • -i)$
where the left-most is 0 but the rightmost is undefined since it is an unbounded
sum.

Quantifications with finite ranges are always defined, and quantifications using
operator ∧ or ∨ are always defined.
...
/y/ does not occur in /R/, and /x/ does not occur in /Q/,
\[ (⊕ x ❙ R • (⊕ y ❙ S • P)) \quad=\quad (⊕ y ❙ S • (⊕ x ❙ R • P)) \]

*(8.20) Axiom, Nesting:* Provided /y/ does not occur in /R/,
\[ (⊕ x, y ❙ R ∧ S • P) \quad=\quad (⊕ x ❙ R • (⊕ y ❙ S • P)) \]

*(8.21) Axiom, Dummy Renaming:* Provided /y/ does not occur in /R/ nor in /P/,
\[ (⊕ x ❙ R • P) \quad=\quad (⊕ y ❙ R[x ≔ y] • P[x ≔ y]] \]

The ‘occurs’ restrictions on these laws ensure that an expression that contains
an occurrence of a dummy is not moved outside (or inside) the scope of the
dummy.

The next rule let's us “shuffle around the possible values being summed over”.

*(8.22) Theorem, Permutative Law, Change of Dummy:* Provided
...
For instance: let $[\_{}] : 𝔹 → ℕ; [true] = 1, [false] = 0$, then,
...
# - *(9.4) Theorem, Trading:* $(∀ x ❙ Q ∧ R • P) ≡ (∀ x ❙ Q • R ⇒ P)$

*** Contextual Rules
    :PROPERTIES:
    :CUSTOM_ID: Contextual-Rules
    :END:
 ...
 *(8.d) Axiom, Distributivity of ⊗ over ⊕:* Provided /x/ does not occur in /P/,
 and “⊗ distributes over ⊕” ---/a ⊗ (b ⊕ c) = (a ⊗ b) ⊕ (a ⊗ c)/ and /a ⊗ Id_⊕ =
 Id_⊕/---
 ...
 For instance,
 - (8.d.Σ) $P · (Σ x ❙ R • Q) \quad=\quad (Σ x ❙ R • P · Q)$.
 - *(9.5) Distributivity of ∨ over ∀:*  $P ∨ (∀ x ❙ R • Q) \quad≡\quad (∀ x ❙ R • P ∨ Q)$.
 - *(9.21) Distributivity of ∧ over ∃:*  $P ∧ (∃ x ❙ R • Q) \quad≡\quad (∃ x ❙ R • P ∧ Q)$.
...
*** Unital Body
    :PROPERTIES:
    :CUSTOM_ID: Unital-Body
    :END:

*(8.u) Axiom, Unital Body:* $(⊕ x ❙ R • Id_⊕) = Id_⊕$

For instance,
- (8.u.Σ): $(Σ x ❙ R • 0) = 0$
- (8.u.Π): $(Π x ❙ R • 1) = 1$
- (8.u.∃) (9.8): $(∀ x ❙ R • true) ≡ true$
- (8.u.∀) (9.24): $(∃ x ❙ R • false) = false$
...
The general axioms of quantification hold for ∧, ∀, along with additional ones
below.
