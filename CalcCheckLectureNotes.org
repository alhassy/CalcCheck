...
1. Textual substitution is inextricably intertwined with equality.

2. Likewise, assignment statements in programming can be reasoned
...
3. The <<<assignment statement>>>
   ...
   commands are relations on predicate*: /Given two propositions $G, R$ making/
   /use of program variables, we write $R \;⇒[C]\;\; G$ to mean “in a state $R$, the
   ...
   | $R \,\;⇒[C]\;\; G$ | ≈ | “Get goal $G$ using command $C$, by requiring $R$.” |
...
** What is the definition of $R \;⇒[C]\;\; G$?
   :PROPERTIES:
...
** We define assignment using textual substitution
   ...
   \[
   \mathsf{Definition\, of\, Assignment:}\quad G[x:= E] \;⇒[x:= E]\;\; G
   \]
